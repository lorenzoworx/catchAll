...
def test_extracts_protected_details() -> None:
    text = "Dr. Steven Archer will not pay $2,000 on August 14."
    ...
    assert extract_negations(text) == {"not": 1}
...
def test_accepts_rewrite_that_preserves_details() -> None:
    ...
    original = "Dr. Steven Archer will not pay $2,000 on August 14."
    candidate = "Dr. Steven Archer won't make the $2,000 payment on August 14."
...
def test_rejects_changed_date() -> None:
    ...
    assert (
        guard.accepts("The appointment is on August 14.", "The appointment is on August 13.")
        is False
    )
...
def test_rejects_changed_name() -> None:
    ...
    assert guard.accepts("Please contact Steven Archer.", "Please contact Saul Archer.") is False


def test_rejects_removed_negation() -> None:
    guard = FaithfulnessGuard()

    assert guard.accepts("The service is not available.", "The service is available.") is False


def test_rejects_added_negation() -> None:
    guard = FaithfulnessGuard()

    assert guard.accepts("The service is available.", "The service is not available.") is False
...
def test_rejects_explicit_start_stop_reversal() -> None:
    ...
    assert (
        guard.accepts("The blue button starts recording.", "The blue button stops recording.")
        is False
    )
...
def test_rejects_before_after_reversal() -> None:
    ...
    assert (
        guard.accepts("The meeting begins before lunch.", "The meeting begins after lunch.")
        is False
    )
...
def test_rejects_storage_deletion_reversal() -> None:
    ...
    assert (
        guard.accepts(
            "Captions are deleted after the session.", "Captions are stored after the session"
        )
        is False
    )
