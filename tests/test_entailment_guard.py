...
@pytest.mark.parametrize(
    ...
    [(-0.1, 0.2), (1.1, 0.2), (0.8, -0.1), (0.8, 1.1)],
)
def test_rejects_invalid_thresholds(
    minimum_entailment: float, maximum_contradiction: float
) -> None:
...
def test_detail_failure_skips_nli() -> None:
    ...
    guard = CompositeGuard(
        FaithfulnessGuard(), ContrastGuard(), BidirectionalEntailmentGuard(scorer)
    )
